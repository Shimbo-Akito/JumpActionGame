...
public class GameScreen extends ScreenAdapter {
    ...
    Sound sound = Gdx.audio.newSound(Gdx.files.internal("se01.mp3"));
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.akito.shimbo.jumpactiongame");
    ...
    public void render(float delta) {
        ...
        for(int i = 0; i < mEnemies.size(); i++){
            if(mEnemies.get(i).mState == Enemy.ENEMY_STATE_VANISH){
                continue;
            }
            boolean flip = (mEnemies.get(i).Direction == Enemy.ENEMY_DIRECTION_LEFT);
            mGame.batch.draw(enemyTexture, mEnemies.get(i).getX(), mEnemies.get(i).getY(), 1.0f, 1.0f, 0, 0, 400, 415, flip, false);
        }
    ...
    private void createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            //敵を配置
            if (y > maxJumpHeight && y < WORLD_HEIGHT - maxJumpHeight * 4) {
                if (mRandom.nextFloat() > 0.7f) {
                    Enemy enemy = new Enemy(enemyTexture, 0, 0, 400, 415);
                    enemy.setPosition(mRandom.nextFloat() * WORLD_WIDTH, y + mRandom.nextFloat() * maxJumpHeight);
                    mEnemies.add(enemy);
                }
            }
